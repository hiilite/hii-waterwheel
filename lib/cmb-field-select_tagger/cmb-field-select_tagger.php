<?php
/*
Plugin Name: CMB2 Field Type: Select_Tagger
...
class PW_CMB2_Field_Select_Tagger {
	...
	public function __construct() {
		...
		add_filter( 'cmb2_render_pw_multitagselect', array( $this, 'render_pw_multitagselect' ), 10, 5 );
		add_filter( 'cmb2_sanitize_pw_multitagselect', array( $this, 'pw_multitagselect_sanitize' ), 10, 4 );
		add_filter( 'cmb2_types_esc_pw_multitagselect', array( $this, 'pw_multitagselect_escaped_value' ), 10, 3 );
		add_filter( 'cmb2_repeat_table_row_types', array( $this, 'pw_multitagselect_table_row_class' ), 10, 1 );
	}
	...
	public function render_pw_select( $field, $field_escaped_value, $field_object_id, $field_object_type, $field_type_object ) {
		...
		echo $field_type_object->select( array(
			'class'            => 'pw_select pw_select_tagger',
	 ...
	 */
	public function render_pw_multitagselect( $field, $field_escaped_value, $field_object_id, $field_object_type, $field_type_object ) {
		$this->setup_admin_scripts();
		...
		$a = $field_type_object->parse_args( 'pw_multitagselect', array(
			'multiple'         => 'multiple',
			'style'            => 'width: 99%',
			'class'            => 'pw_select_tagger pw_multitagselect',
			'name'             => $field_type_object->_name() . '[]',
			'id'               => $field_type_object->_id(),
			'desc'             => $field_type_object->_desc( true ),
			'options'          => $this->get_pw_multitagselect_options( $field_escaped_value, $field_type_object ),
			'data-placeholder' => $field->args( 'attributes', 'placeholder' ) ? $field->args( 'attributes', 'placeholder' ) : $field->args( 'description' ),
		) );
	...
	/**
	 * Return list of options for pw_multitagselect
	 *
	 ...
	 */
	public function get_pw_multitagselect_options( $field_escaped_value = array(), $field_type_object ) {
	 ...
	 */
	public function pw_multitagselect_sanitize( $check, $meta_value, $object_id, $field_args ) {
	 ...
	 */
	public function pw_multitagselect_escaped_value( $check, $meta_value, $field_args ) {
		if ( ! is_array( $meta_value ) || ! $field_args['repeatable'] ) {
			return $check;
		}
	 ...
	 */
	public function pw_multitagselect_table_row_class( $check ) {
		$check[] = 'pw_multitagselect';
	...
	public function setup_admin_scripts() {
		$asset_path = apply_filters( 'pw_cmb2_field_select_tagger_asset_path', plugins_url( '', __FILE__  ) );

		wp_register_script( 'pw-select_tagger', $asset_path . '/js/select2.min.js', array( 'jquery-ui-sortable' ), '4.0.3' );
		wp_enqueue_script( 'pw-select_tagger-init', $asset_path . '/js/script.js', array( 'cmb2-scripts', 'pw-select_tagger' ), self::VERSION );
		wp_register_style( 'pw-select_tagger', $asset_path . '/css/select2.min.css', array(), '4.0.3' );
		wp_enqueue_style( 'pw-select_tagger-tweaks', $asset_path . '/css/style.css', array( 'pw-select_tagger' ), self::VERSION );
	}
}
